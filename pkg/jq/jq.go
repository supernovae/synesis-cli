...
// ApplyWithGoJq applies a jq-style filter using the gojq library for complex queries.
func ApplyWithGoJq(content string, filter string) (string, error) {
	var data interface{}
	if err := json.Unmarshal([]byte(content), &data); err != nil {
		return "", fmt.Errorf("parse json: %w", err)
	}

	// For simple field access, use the custom parser
	if !needsGoJq(filter) {
		return Apply(content, filter)
	}

	// For complex queries, use gojq
	...
	// If single result, return it; otherwise return array
...
// needsGoJq checks if a filter requires the gojq library (complex queries).
func needsGoJq(filter string) bool {
	f := strings.TrimSpace(filter)
	// Complex patterns that require gojq
